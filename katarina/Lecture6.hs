...
exM :: Integer -> Integer -> Integer -> Integer
exM _ 0 base = mod 1 base
exM x 1 base = mod x base
exM x y base = let subtree = exM x (div y 2) base in 
               if even y 
               then mod (subtree * subtree) base               
               else mod (x * subtree * subtree) base

--helperF x y base = mod ((helperF x (div y 2) base) * (helperF x (div y 2) base)) base
